...
class SellerSubCategoryService extends SubCategoriesService{
  SellerSubCategoryService(super.apiService);
  @override
  Future<Either<Failure, List<SubCategoryModel>>> getSubCategories() async {
    final response = await apiService.getRequest(
      '${baseurl}seller/${AppEndpoints.getSubCategories}',
  ...
  Future<Either<Failure, SubCategoryModel>> getSubCategoriesById(
    ...
    final response = await apiService.getRequest(
      '${baseurl}seller/${AppEndpoints.getSubCategories}/$categoriesId',
      headers: {'Authorization': "Bearer ${LocalStorageService.getToken()}"},
    );
    if (response.statusCode == 200) {
